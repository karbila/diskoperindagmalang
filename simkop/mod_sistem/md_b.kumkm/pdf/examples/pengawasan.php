...
$t = $_GET['th'];
$b = getBulan($_GET['bl']);
$j = "SELECT r.kode_penga, r.nama_penga, r.tgl_awal_penga, r.tgl_akhir_penga, r.ang_penga, r.detil_penga, r.tempat_penga, p.nama_pegawai
FROM tbl_pengawasan r, pegawai p
WHERE r.id_pegawai = p.id_pegawai
AND SUBSTRING( r.tgl_awal_penga, 1, 4 ) =  '$t'
AND SUBSTRING( r.tgl_akhir_penga, 6, 2 ) =  '$_GET[bl]'
ORDER BY r.kode_penga ASC";
$q_ren = mysql_query($j);
$jum = mysql_num_rows($q_ren);

$coba = '<br><h2 style="text-align:center">Laporan Pengawasan Bulan '.$b.' Tahun '.$t.'</h2><br>
<table border="1" cellpadding="4">
              ...
              <tbody>';                
                ...
                while ($d=mysql_fetch_array($q_ren)) {
                  $start = dbtoindo($d[2]);
                  $finish = dbtoindo($d[3]); 
                  $uang = number_format($d['ang_penga'],2,",",".");                 
                  $coba .="<tr>";
