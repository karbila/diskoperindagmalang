...
$pdf->SetTitle('Laporan Pelaksanaan');
...
$t = $_GET['th'];
$b = getBulan($_GET['bl']);
$j = "SELECT r.kode_laksana, r.nama_laksana, r.tgl_m_lak, r.tgl_s_lak, r.ang_lak, r.des_lak, r.tempat_lak, p.nama_pegawai
FROM tbl_pelaksanaan r, pegawai p
WHERE r.id_pegawai = p.id_pegawai
AND SUBSTRING( r.tgl_m_lak, 1, 4 ) =  '$t'
AND SUBSTRING( r.tgl_s_lak, 6, 2 ) =  '$_GET[bl]'
ORDER BY r.kode_laksana ASC";
$q_ren = mysql_query($j);
$jum = mysql_num_rows($q_ren);

$coba = '<br><h2 style="text-align:center">Laporan Pelaksanaan Bulan '.$b.' Tahun '.$t.'</h2><br>
<table border="1" cellpadding="4">
              ...
              <tbody>';                
                ...
                while ($d=mysql_fetch_array($q_ren)) {
                  $start = dbtoindo($d[2]);
                  $finish = dbtoindo($d[3]); 
                  $uang = number_format($d['ang_lak'],2,",",".");                 
                  $coba .="<tr>";
                  $coba .="<td style='width:10px;'>$d[0]</td><td>$d[1]</td><td>$start</td><td>$finish</td><td>$d[6]</td><td>$d[7]</td><td>Rp. $uang</td><td>$d[5]</td>";
                  $coba .="</tr>";
                }
...
$now = date('Y-m-d');
$tg = tgl_indo($now);
$i = mysql_fetch_array(mysql_query("SELECT p.*, j.nama_jab FROM pegawai p JOIN jabatan j ON j.id_jab=p.id_jab WHERE id_pegawai = '6'")); // staf
$tdd = "<table>
		<tr>
		<td></td>
		<td></td>
...
$info  = "<br><br><br><br><br><br><hr>Jumlah Data Pelaksanaan = ".$jum." Data<br>Dicetak Oleh = ".$_SESSION['namauser']." - ".$_SESSION['namajab']."<br>".$current_date;
...
$pdf->Output('Laporan Pelaksanaan.pdf', 'I');
//============================================================+
// END OF FILE
//============================================================+
