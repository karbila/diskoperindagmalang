<!-- awal tabel -->
  <div class="span12" id='tbl_rencana' style='margin-left:0px'>
      <div class='page-header'>
          ...
          $uu = "SELECT r.id_rencana, r.kode_ren, r.nama_keg_ren, r.mulai_ren, r.selesai_ren, r.tempat_ren, p.nama_pegawai, r.anggaran_ren, r.desk_ren, d.*
                 FROM tbl_rencana r
                 JOIN pegawai p ON r.id_pegawai = p.id_pegawai
                 JOIN jabatan j ON j.id_jab = p.id_jab
                 JOIN dokumen d ON d.id_dok = r.id_dok
                 WHERE r.id_rencana NOT IN ($y)
                 ";
          
          //echo "$uu";
          
          $q_ren = mysql_query($uu);
          $jumrenn = mysql_num_rows($q_ren);
           ?>
          <h3>Perencanaan (Planning) - <span class='badge badge-info'><?php echo "$jumrenn"; ?></span> Data yang belum direalisasi sampai Bulan <?php echo "$b"; ?></h3>
        </div>        
